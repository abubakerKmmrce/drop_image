	...
	filesData.forEach(async (file) => {
		...
		if (!file.uploaded) {
			...
			//set server id to file
			 file.id = response.body.data.id;
			 console.log('file.id',response.body.data.id)
		}
	});
}
...
async function saveImageToServer(file) {
	return new Promise(async (resolve, reject) => {
		...
		const response = await (new APIHelper()).post('/create', formData, { 'Authorization': `Bearer 1|hV9Hk6P1azENWQc1psJ1I7z6dLBS6Bprbovwi2yV`, 'X-CSRFToken': xcrf });
...
//		console.log('delete function:', fileId, filesData[0].id)
		const response = await (new APIHelper()).post('/delete/'+filesData[0].id, {}, { 'Authorization': `Bearer 1|hV9Hk6P1azENWQc1psJ1I7z6dLBS6Bprbovwi2yV`, 'X-CSRFToken': xcrf  });
		//clear rendering area
		clearGallery();
		//*loop through the files and preview them
...
//	const file = filesData.find((file) => file.id == fileID);
	...
	console.log(fileID,filesData,file)
	return new Promise(async (resolve, reject) => {
		//prepare the data to send to the server
		const formData = new FormData();
		formData.append('tag', file.tag);
		...
		const response = await (new APIHelper()).post('/update/'+file.id, formData, { 'Authorization': `Bearer 1|hV9Hk6P1azENWQc1psJ1I7z6dLBS6Bprbovwi2yV` , 'X-CSRFToken': xcrf });
		resolve(response);
	});
}