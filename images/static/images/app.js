...
function registerUpdateButtonsLeinster() {
	...
	updateButtons.forEach(button => {
		button.addEventListener('click', async function (event) {
            ...
            let updated_file
            filesData.forEach(myFunction);
            function myFunction(item, index) {
            console.log('loop',item.id, fileId,index)

            if (fileId == index +1) {
            updated_file = {"id":item.id, "tag":fileTag, "name":fileName}
            console.log('new file name:', updated_file)
			...
			await updateFile(fileId, updated_file);
